...
for (i in 1:nrow(individual)) {
  ...
  write(paste("/lustre/work/johruska/msmc_2.0.0_linux64bit -o ./", individual[i,], "/msmc_output/bootstraps/bootstrap_${SLURM_ARRAY_TASK_ID} -t 8 -i 20  -p 1*2+20*1+1*2+1*3 ./", individual[i,], "/multihetsep/bootstraps/bootstrap_${SLURM_ARRAY_TASK_ID}/*.txt -I 0,1", sep = ""), file=a.script[i], append=T)
