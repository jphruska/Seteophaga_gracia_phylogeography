options(scipen=999)
project_directory <- "/lustre/scratch/johruska/setophaga/03_vcf/combined_vcfs/dataset17_div_diff"
...
ref_index <- read.table("GCA_001746935.2_mywa_2.1_genomic.fasta.fai", stringsAsFactors=F)
...
header <- paste('\tgunzip -cd ', project_directory, '/CM027507.1.g.vcf.dataset17.diff.div.filtered.vcf.recode.vcf.gz | grep "#" > ', project_directory, "/windows/${chrom_array}__${start_array}__${end_array}.recode.vcf", sep="")
