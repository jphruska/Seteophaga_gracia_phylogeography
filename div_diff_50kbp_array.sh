...
for (( run=$START_NUM; run<=$END_NUM; run++ )); do
	...
	gunzip -cd /lustre/scratch/johruska/setophaga/03_vcf/combined_vcfs/dataset1_div_diff/CM027507.1.g.vcf.phylo.filtered.vcf.recode.vcf.gz | grep "#" > /lustre/scratch/johruska/setophaga/03_vcf/combined_vcfs/dataset1_div_diff/windows/${chrom_array}__${start_array}__${end_array}.recode.vcf

	tabix /lustre/scratch/johruska/setophaga/03_vcf/combined_vcfs/dataset1_div_diff/${chrom_array}.recode.vcf.gz ${chrom_array}:${start_array}-${end_array} >> /lustre/scratch/johruska/setophaga/03_vcf/combined_vcfs/dataset1_div_diff/windows/${chrom_array}__${start_array}__${end_array}.recode.vcf
